            ...
            for (int x = 0; x < numVertsPerLine; x ++)
            {
                ...
                if (!isSkippedVertex)
                {
                    ...
                    if (isEdgeConnectionVertex)
                    {
                        ...
                        float heightOfMainVertexA = heightMap[(isVertical ? x : x - distToMainVertexA),
                            (isVertical ? y - distToMainVertexA : y)];
                        float heightOfMainVertexB = heightMap[(isVertical ? x : x + distToMainVertexB),
                            (isVertical ? y + distToMainVertexB : y)];

                        height = heightOfMainVertexA * (1 - distPercentFromAToB) +
                                 heightOfMainVertexB * distPercentFromAToB;
                    }
